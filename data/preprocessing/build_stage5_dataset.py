...
def main() -> None:
    ...
    for split in ("train", "val", "test"):
        ...
        phi, ers = build_phi_and_targets(features)
        np.save(output_dir / f"{split}_features.npy", phi.astype(np.float32))
...
def build_phi_and_targets(seq_features: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    for w in windows:
        ...
        phi = np.array([f["F1"], f["F2"], 1.0 - f["F3"], f["F4"]], dtype=np.float32)
        # Proxy ERS target while human EQ supervision is not yet available.
        target = float(np.clip(0.28 * phi[0] + 0.34 * phi[1] + 0.18 * phi[2] + 0.20 * phi[3], 0.0, 1.0))
        phi_rows.append(phi)
        targets.append(target)
    return np.stack(phi_rows).astype(np.float32), np.array(targets, dtype=np.float32)
